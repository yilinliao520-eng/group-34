...
\usepackage{times}
\usepackage{enumitem}
\title{Fine-Tuned and Controller-Guided LLM Agent for WebShop}
\author{Group 34}
\date{}
\begin{document}
\maketitle
...
We use WebShop full mode with 1.18M products and a full Lucene search index. The agent receives a shopping instruction and interacts with a text-mode environment using actions such as \texttt{search[keywords]}, \texttt{click[ASIN]}, and \texttt{click[Buy Now]}.
...
\paragraph{Anti-Loop Controller.}
...
\bottomrule
\end{tabular}
\caption{Main 50-goal full synthetic evaluation.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Controller ablation for the unified SFT model.}
\end{table}
