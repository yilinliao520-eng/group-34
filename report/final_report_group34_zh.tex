...
WebShop 是一个面向真实电商交互的 benchmark，包含大规模商品库、自然语言 shopping goals 和基于 reward 的评价方式 \citep{yao2022webshop}。ReAct 框架通过交替生成 reasoning 和 action，使语言模型能够在交互环境中逐步决策 \citep{yao2022react}。在偏好学习方面，DPO 将 preference optimization 转化为直接优化 chosen/rejected response 的目标函数 \citep{rafailov2023dpo}，理论上适合修正 WebShop 中的局部动作偏好，例如 repeated search 与 click 之间的选择。
